import { useState } from 'react'
import ThemeToggle from './ThemeToggle'

function StartScreen({ onLoginClick, onSignupClick, onGoogleSignIn }) {
  const [isGoogleLoading, setIsGoogleLoading] = useState(false)

  const handleGoogleSignIn = async () => {
    try {
      setIsGoogleLoading(true)
      await onGoogleSignIn()
    } catch {
      // Error feedback is handled by the parent auth flow.
    } finally {
      setIsGoogleLoading(false)
    }
  }

  return (
    <div className="min-h-screen flex items-center justify-center p-6 gradient-bg transition-colors duration-500">
      <div className="absolute top-5 right-5 animate-fade-in">
        <ThemeToggle />
      </div>
      
      <div className="w-full max-w-lg animate-scale-in">
        <div className="glass p-10 sm:p-12">
          <div className="text-center space-y-3">
            <h1 className="text-5xl sm:text-6xl font-bold text-slate-900 dark:text-white tracking-tight">
              SkillForge
            </h1>
            <p className="text-lg text-slate-600 dark:text-gray-400 font-350">Your learning playground.</p>
          </div>

          <div className="mt-12 space-y-4">
            <button
              className="w-full btn-outline flex items-center justify-center gap-3 hover:-translate-y-0.5 active:translate-y-0"
              onClick={handleGoogleSignIn}
              disabled={isGoogleLoading}
              type="button"
            >
              <svg className="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              <span>{isGoogleLoading ? 'Signing in...' : 'Continue with Google'}</span>
            </button>

            <div className="relative">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t border-slate-200 dark:border-gray-700"></div>
              </div>
              <div className="relative flex justify-center text-sm">
                <span className="px-3 bg-white dark:bg-gray-800 text-slate-500 dark:text-gray-400 font-350">or</span>
              </div>
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
              <button
                className="btn-primary hover:-translate-y-0.5 active:translate-y-0"
                onClick={onLoginClick}
              >
                Login
              </button>

              <button
                className="btn-secondary hover:-translate-y-0.5 active:translate-y-0"
                onClick={onSignupClick}
              >
                Create Account
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

export default StartScreen

