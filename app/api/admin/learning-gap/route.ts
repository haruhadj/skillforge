...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [usersSnap, scoresSnap, statsSnap] = await Promise.all([
      adminDb.collection('users').get(),
      adminDb.collectionGroup('scores').get(),
      adminDb.collectionGroup('gameStats').get(),
    ])

    const users: AnalyticsUserRow[] = usersSnap.docs.map((d: QueryDocumentSnapshot<DocumentData>) => {
      const data = d.data()
      return {
        uid: d.id,
    ...
    const scores: AnalyticsScoreRow[] = scoresSnap.docs.map((d: QueryDocumentSnapshot<DocumentData>) => ({
      uid: uidFromPath(d.ref.path),
      gameId: d.id,
      bestScore: Number(d.data().bestScore) || 0,
    }))

    const stats: AnalyticsStatsRow[] = statsSnap.docs.map((d: QueryDocumentSnapshot<DocumentData>) => ({
      uid: uidFromPath(d.ref.path),
      gameId: d.id,
